...
int main (int argc, char ** argv) 
{
    ...
    print0("adios_read_init_method() rank %d time %lf\n", rank, tm_end-tm_st);
    ...
    f = adios_read_open (infilename, read_method, comm, 
                         ADIOS_LOCKMODE_NONE, timeout_sec);
    tm_end = timer_read(&timer_);
    ...
    {
        print0("adios_read_open() rank %d time %lf\n", rank, tm_end-tm_st);
        // process steps here... 
        if (f->current_step != 0) {
        ...
        adios_read_close (f);
        tm_end = timer_read(&timer_);
        print0("adios_read_close() rank %d time %lf\n", rank, tm_end-tm_st);
    ...
    tm_end = timer_read(&timer_);
    print0("adios_read_finalize_method() rank %d time %lf\n", rank, tm_end-tm_st);
...
int process_metadata(int step)
{
    ...
    double tm_st, tm_end, tm_sum = 0.0, tm_max;
    int root = 0;
    ...
    MPI_Reduce(&tm_sum, &tm_max, 1, MPI_DOUBLE, MPI_MAX, root, comm);
    if (rank == root) {
        print0("TS= %u TRANSPORT_TYPE= %s get Metadata MAX time= %lf\n",
                step, rmethodname, tm_max);
    }
...
int read_write(int step)
{
    ...
    MPI_Reduce(&tm_sum, &tm_max, 1, MPI_DOUBLE, MPI_MAX, root, comm);
    if (rank == root) {
        print0("TS= %u TRANSPORT_TYPE= %s write MAX time= %lf\n",
                step, rmethodname, tm_max);
    }
