...
void calc_pv() {
  ...
  MPI_Allreduce(lpv,pv,NBIN,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
...
/* Main loop over x, y & z directions starts--------------------------*/
  ...
  for (kd=0; kd<3; kd++) {
    ...
    for (kdd=0; kdd<2; kdd++) {
      ...
      MPI_Isend(dbuf,3*nsd,MPI_DOUBLE,inode,20,MPI_COMM_WORLD,&requests);
      MPI_Wait(&requestr, &status);
...
----------------------------------------------------------------------*/
  ...
  for (kd=0; kd<3; kd++) {
    ...
    for (kdd=0; kdd<2; kdd++) {
      ...
      MPI_Isend(dbuf,6*nsd,MPI_DOUBLE,inode,120,MPI_COMM_WORLD,&requests);
      MPI_Wait(&requestr, &status);
